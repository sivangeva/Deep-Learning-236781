...
class FirstLastSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        # ====== YOUR CODE: ======
        for index in range(int((len(self.data_source)+1)/2)):
            if index == len(self.data_source)-(1+index):
                yield index
            else:
                yield index
                yield len(self.data_source)-(1+index)
...
def create_train_validation_loaders(
    ...
    train_idx, val_idx = indices[int(0.2 * len(dataset)):], indices[:int(0.2 * len(dataset))]
    train_sampler = torch.utils.data.SubsetRandomSampler(train_idx)
    val_sampler = torch.utils.data.SubsetRandomSampler(val_idx)
    dl_train = torch.utils.data.DataLoader(dataset=dataset, shuffle=False, batch_size=batch_size, sampler=train_sampler)
    dl_valid = torch.utils.data.DataLoader(dataset=dataset, shuffle=False, batch_size=batch_size, sampler=val_sampler)
